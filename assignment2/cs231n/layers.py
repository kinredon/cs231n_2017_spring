...
def affine_backward(dout, cache):
    ...
    x_transpose = np.reshape(x, (x.shape[0], -1)).T
    dw = x_transpose.dot(dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, eps, gamma, beta, norm_x, sample_mean, sample_var)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, eps, gamma, beta, norm_x, running_mean, running_var = cache
    N,D = x.shape
    dnorm_x = dout * gamma
    drunning_var = np.sum(dnorm_x*( x - running_mean),axis=0)*(-0.5)*(running_var+eps)**(-3.0/2.0)
    drunning_mean = np.sum(dnorm_x*(-1.0/ np.sqrt(running_var + eps)) ,axis=0) + drunning_var*(np.sum(-2*(x - running_mean), axis=0) / N)
    dx = dnorm_x * (1.0 / np.sqrt(running_var + eps))+drunning_var*2*(x - running_mean)/N + drunning_mean * 1.0 / N
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # 1 - p means probability of keeping a unit active 
        ...
        for ci in range(C):
            for hi in range(HH):
                for wi in range(WW):
                    ...
                    max_idx= int(max_id / pool_height) + hi * stride
                    max_idy= max_id % pool_width + wi * stride
                    dx[ni, ci, max_idx, max_idy] += dout[ni, ci, hi, wi]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
